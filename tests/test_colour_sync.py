...
from app.constants import COLOURS as APP_COLOURS

# server/ uses flat imports, so add it to the path to load room.py directly
# (the server can't import app/, hence the duplicated list this test guards).
...
def test_badge_and_server_colours_match():
    # The badge maps these names to RGB (app/constants.py) and the server
